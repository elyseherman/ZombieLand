...
import ZombieLand.tiles.PlainTile;
import ZombieLand.tiles.DesertTile;
import ZombieLand.tiles.MountainTile;
import ZombieLand.tiles.FacilityTile;
import ZombieLand.tiles.MetroTile;
import ZombieLand.tiles.ZombieInfectedRuinTile;
...
public class MiniTester {
    ...
    public static boolean testSPathGenerateGraph1() { // 2 points
        ...
        if (edges.size() != 8) {
            System.out.println("There should be 8 edges, but there are "+edges.size());
            return false;
        }
    ...
    public static boolean testSPathGenerateGraph2() { // 3 points
        ...
        ArrayList<Graph.Edge> edges = shortest.g.getAllEdges();
        if (edges.size() != 40) {
            System.out.println("There should be 40 edges, but there are "+edges.size()+ ". Note that MountainTile is not reachable.");
            return false;
        }
        for (Graph.Edge e: edges) {
            if (e.weight != e.destination.distanceCost) {
